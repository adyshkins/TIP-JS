...
export function validateTaskInput(value, { partial = false, categoryIds } = {}) {
  ...
  if (!(categoryIds instanceof Set)) {
    throw new TypeError('categoryIds должен быть Set.');
  }
  ...
  for (const name of Object.keys(value)) {
    if (!allowed.has(name)) {
      details[name] = 'Неизвестное или запрещённое поле.';
    }
  }
  ...
  if (Object.hasOwn(value, 'completed')) {
    if (typeof value.completed !== 'boolean') {
      details.completed = 'Статус должен быть boolean.';
    } else {
