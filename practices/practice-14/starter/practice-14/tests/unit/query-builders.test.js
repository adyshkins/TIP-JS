...
test('escapeLikePattern экранирует %, _ и обратную косую черту', () => {
  assert.equal(escapeLikePattern('50%_готово\\да'), '50\\%\\_готово\\\\да');
});

test('SELECT не помещает пользовательский q в SQL-текст', () => {
  const query = buildListTasksQuery({ ...empty, q: "x%' OR true --" });
  assert.doesNotMatch(query.text, /OR true|--/);
  assert.deepEqual(query.values, ["%x\\%' OR true --%"]);
});

test('UPDATE одного completed сохраняет значение false', () => {
...
test('UPDATE отклоняет пустой и неизвестный набор изменений', () => {
  assert.throws(() => buildUpdateTaskQuery(1, {}), /поле изменения/i);
  assert.throws(() => buildUpdateTaskQuery(1, { role: 'admin' }), /неизвестное поле/i);
});
