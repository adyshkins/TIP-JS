...
test('OPTIONS: 204 без тела и CORS чтения', async () => {
  ...
  assert.equal(r.response.headers.get('access-control-allow-methods'), 'GET, HEAD, POST, PATCH, DELETE, OPTIONS');
});
for (const [method, path] of [['PUT','/api/tasks'], ['POST','/api/tasks/4'], ['PUT','/api/categories'], ['POST','/api/health']]) {
  test(`${method} ${path}: 405 и Allow`, async () => {
    const r = await request(path, { method });
    checkError(r, 405, 'METHOD_NOT_ALLOWED');
    const allow = path === '/api/tasks' ? 'GET, HEAD, POST, OPTIONS' : path === '/api/tasks/4' ? 'GET, HEAD, PATCH, DELETE, OPTIONS' : 'GET, HEAD, OPTIONS';
    assert.equal(r.response.headers.get('allow'), allow);
  });
}
...
test('Фильтрация и запрещённый PUT не изменяют исходный набор', async () => {
  await request('/api/tasks?completed=false');
  await request('/api/tasks/4', { method: 'PUT' });
  const r = await request('/api/tasks');
