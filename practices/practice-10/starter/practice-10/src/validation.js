...
function oneValue(searchParams, name) {
  ...
  if (values.length > 1) throw new ApiError(400, 'INVALID_QUERY', `Параметр ${name} нельзя повторять.`);
...
export function parseFilters(searchParams) {
  for (const name of searchParams.keys()) {
    if (!FILTER_NAMES.has(name)) throw new ApiError(400, 'INVALID_QUERY', `Неизвестный параметр запроса: ${name}.`);
  }
  ...
  if (priority !== null) {
    if (!PRIORITIES.has(priority)) throw new ApiError(400, 'INVALID_QUERY', "priority должен быть low, medium или high.");
  ...
  if (categoryId !== null) {
    ...
    if (parsed === null) throw new ApiError(400, 'INVALID_QUERY', "categoryId должен быть положительным целым числом.");
  ...
  if (q !== null) {
    filters.q = q.trim();
    if (filters.q.length > 100) throw new ApiError(400, 'INVALID_QUERY', "q не должен быть длиннее 100 символов.");
  }
  return filters;
}
